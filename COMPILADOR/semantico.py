...
def ConferirDeclaracao (valores):
	if valores[2] != '':
		return True
	else:
		return False
...
def avalia(prod, qtd_smb):
	...
	#pilha_semantica.append(lado_esq)
	...
	if prod == gram[5]:   #LV -> varfim
		...
		i = -1
		while(pilha_semantica[i][2] not in ['inteiro','real','literal']):
			if ConferirDeclaracao(pilha_semantica[i]):
				print("Variavel já foi declarada na linha: "+ str(Analisador_lexico.linha), " coluna : " +str(Analisador_lexico.coluna))
				erro = True
			i -= 1
		
	elif prod == gram[8]: #L -> id 
		aux = pilha_semantica.pop()
		if ConferirDeclaracao(aux):
			print("Variavel já foi declarada na linha: "+ str(Analisador_lexico.linha), " coluna : " +str(Analisador_lexico.coluna))
			erro = True
		else:
			pilha_semantica.append(aux)
		...
		if ConferirDeclaracao(pilha_semantica[-1]):
		...
		if not ConferirDeclaracao(pilha_semantica[-1]):
			print("Variável não declarada linha:"+str(Analisador_lexico.linha), "coluna : " + str(Analisador_lexico.coluna))
			erro = True
		...
		text = str(x1) + ' T' + str(numTemp) + ' = ' + str(pilha_semantica[-3][1]) + '' + str(pilha_semantica[-1][1]) + '' + str(pilha_semantica[-2][1]) + ';' 
		addbuff(text)
	...
	#elif prod == gram[33]: #R -> facaAte (EXP_R) CP_R
